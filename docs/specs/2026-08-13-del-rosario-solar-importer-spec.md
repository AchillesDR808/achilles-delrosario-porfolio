# U.S. Solar Equipment Importer — FX Hedge Model Technical Specification
...
| Date | 2026-08-13 |
| Version | 0.2 |
| Scenario | 1 — U.S. Solar Equipment Importer (`solar-importer`) |
...
U.S. Solar Equipment Importer expects to receive **EUR 4,500,000** on **2027-08-13** (365 days from inception) and reports in USD. The EUR amount is fixed, but its USD value is not: a weaker euro reduces cash available for the budget, margin, and operating commitments. The workbook will compare unhedged proceeds with forward, money-market, and put-option hedges so the CFO can weigh certainty, liquidity use, and upside participation.
...
| `F0_in` | 1.0875 | USD per EUR, 1-year forward | Assigned scenario parameter; replace with executable bank/dealer or Refinitiv/Bloomberg quote at Stage 4 |
| `R_USD` | 4.25% | Annual %, ACT/360 | U.S. Treasury 1-year yield or USD money-market rate; access date logged |
| `R_FC` | 5.43% | Annual %, ACT/360 placeholder calibrated to the indicative forward | ECB euro-area money-market/EUR deposit rate; access date logged |
...
3. If `S_T ≤ K_PUT`, `USD_PUT = FC_AMT × K_PUT − FC_AMT × PREM_PUT`; if `S_T > K_PUT`, it equals `FC_AMT × S_T − FC_AMT × PREM_PUT`. The two expressions agree at `S_T = K_PUT`, so proceeds are continuous.
...
|---|---|---|---|
| 0.2 | 2026-08-13 | Achilles Del Rosario | Corrected to assigned Scenario 1: U.S. Solar Equipment Importer |
